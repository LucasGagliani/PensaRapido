def mostrarReglas():
    print("\n" + "=" * 60)
    print("🎉  ¡BIENVENIDO A PENSÁ RÁPIDO!  🎉")
    print("=" * 60)
    
    print("\n📜 REGLAS DEL JUEGO:")
    print("🧠 - Juego de preguntas y respuestas.")
    print("🗂️  - Las preguntas están organizadas por categorías y niveles.")
    print("🎯 - Elegí una categoría y dificultad para responder.")
    print("🔢 - Ingresá el número de la opción que consideres correcta.\n")

    print("🎮 MODOS DE JUEGO:")

    print("\n⏱️  Modo por Tiempo:")
    print("   🔄 Responde preguntas aleatorias antes de que se acabe el tiempo.")
    print("   ❌ Las respuestas incorrectas agregan +10 segundos.")
    print("   🏆 Se muestra un ranking con los 20 mejores tiempos.")

    print("\n🤜🤛 Modo 1 vs 1:")
    print("   👥 Dos jugadores se turnan para responder.")
    print("   🏅 Puntos por respuesta correcta:")
    print("     🟢 Fácil: +10 puntos")
    print("     🟡 Media: +20 puntos")
    print("     🔴 Difícil: +30 puntos")
    print("   🏆 Gana quien tenga más puntos al final.\n")

    print("📈 RANKING:")
    print("📝 Al finalizar el modo por tiempo, tu tiempo se guarda en 'puntuaciones.csv'.")

    print("🏅 Se muestran los 20 mejores tiempos y tu posición actual además de los mejores rankeados del 1vs1.\n")

    print("🎊 ¡Divertite y pensá rápido!")
    print("=" * 60 + "\n")
    
    input("🔄 Presiona Enter para volver al menú...") 
